"""
This is the main script which is trying to clean and input data in the correct
order into the internal input sheet.
...
def set_inputdataformat(config_file):
    # this function uses the input data function given in a configration file
    # and converts into a dict of renamed cols which will be used to change
    # the format from your internal format to a CFARS specific one
...
def get_inputdata(filename, config_file):
    ...
    # the input data can be internal structure and does not need to be a set
    # format. The formatting information of the data is provided in the config
    # file, the config file template is in the git hub repo
    ...
    if filename.split('.')[-1] == 'csv':
        ...
        print('Unkown input file type for the input data, '
              'please consider changing it to csv')
        sys.exit()
    ...
    # this needs to be thought through, in essence we are applying a
    # wholesale drop for non exisiting data
    inputdata = inputdata.dropna()
    # this acts as bin because the bin defination is between the two half
    # integer values
    inputdata['bins'] = inputdata['Ref_WS'].round(0)

    # this is creating a interval range of .5 starting at .25
    bins_p5_interval = pd.interval_range(start=.25, end=20, freq=.5,
                                         closed='left')
    out = pd.cut(x=inputdata['Ref_WS'], bins=bins_p5_interval)
    # the middle of the interval is used as a catagorical label
    inputdata['bins_p5'] = out.apply(lambda x: x.mid)
    # we can only analyze where the ref_TI is not 0
    inputdata = inputdata[inputdata['Ref_TI'] != 0]
...
#     # plt = representative_TI[repcols].plot(title='Rep TI plot,
#     #           TI_mean_bin+TI_sd_bin*1.28', ylable='Rep TI')
#     # plt.figure.savefig('./{}_rep_TIplot.png'.format(projectname))

# def get_regression_sm(x, y):
#     # get the linear least squaes fit, this uses statsmodel which needs to
#     # be installed outside of Anaconda
#
...
def get_regression(x, y):
    # get the linear least squaes fit, this uses the sklearn model
    # which is in Anaconda already.
...
def get_ws_regression(inputdata):
    ...
    # this is to get the rsd and ref anemometer results.
    results_rsd_ref = get_regression(inputdata['RSD_WS'], inputdata['Ref_WS'])
    results.loc['WS_regression_RSD_Ref'] = results_rsd_ref

    # this is checking if corrected TI windspeed is present in the
    # input data and using that for getting the results.
    if 'corrTI_RSD_WS' in inputdata.columns:
        results_corrTI_rsd_ref = get_regression(inputdata['corrTI_RSD_WS'],
                                                inputdata['Ref_WS'])
    ...
    if 'corrWS_RSD_WS' in inputdata.columns:
        results_corrWS_rsd_ref = get_regression(inputdata['corrTI_RSD_WS'],
                                                inputdata['Ref_WS'])
    ...
    results_Ane2_ref = get_regression(inputdata['Ane2_WS'],
                                      inputdata['Ref_WS'])
...
def get_representative_TI(inputdata):
    ...
    representative_TI = (inputdata[['RSD_TI', 'Ref_TI', 'Ane2_TI', 'bins']]
                         .groupby(by=['bins'])
                         .agg(['mean', 'std',
                               lambda x: x.mean()+1.28*x.std()]))
    representative_TI.columns = ['RSD_TI_mean', 'RSD_TI_std', 'RSD_TI_rep',
                                 'Ref_TI_mean', 'Ref_TI_std', 'Ref_TI_rep',
                                 'Ane2_TI_mean', 'Ane2_TI_std', 'Ane2_TI_rep']
    return representative_TI


def get_count_per_WSbin(inputdata, column):
    # Count per wind speed bin
    inputdata = inputdata[(inputdata['bins_p5'].astype(float) > 1.5)
                          & (inputdata['bins_p5'].astype(float) < 21)]
    resultsstats_bin = (inputdata[[column, 'bins']]
                        .groupby(by='bins')
                        .agg(['count']))
    resultsstats_bin_p5 = (inputdata[[column, 'bins_p5']]
                           .groupby(by='bins_p5').agg(['count']))
...
def get_stats_per_WSbin(inputdata, column):
    # this will be used as a base function for all frequency agg
    # caliculaitons for each bin to get the stats per wind speed bins
    inputdata = (inputdata[(inputdata['bins_p5'].astype(float) > 1.5)
                           & (inputdata['bins_p5'].astype(float) < 21)])
    resultsstats_bin = (inputdata[[column, 'bins']]
                        .groupby(by='bins').agg(['mean', 'std']))
    resultsstats_bin_p5 = (inputdata[[column, 'bins_p5']]
                           .groupby(by='bins_p5').agg(['mean', 'std']))
    resultsstats_bin = pd.DataFrame(resultsstats_bin.unstack()).T
    resultsstats_bin.index = [column]
    resultsstats_bin_p5 = pd.DataFrame(resultsstats_bin_p5.unstack()).T
    resultsstats_bin_p5.index = [column]
...
def get_TI_MBE_Diff_j(inputdata):
    ...
    inputdata['TI_diff_RSD_Ref'] = inputdata['RSD_TI']-inputdata['Ref_TI']
    # calculating the error for each timestamp
    inputdata['TI_error_RSD_Ref'] = (inputdata['TI_diff_RSD_Ref']
                                     / inputdata['Ref_TI'])
    ...
    # get the bin wise stats for both diff and error between RSD corrected
    # for TI and Ref

    # this is checking if corrected TI windspeed is present in the input
    # data and using that for getting the results.
    if 'corrTI_RSD_TI' in inputdata.columns:
        # caliculating the diff in ti for each timestamp
        inputdata['TI_diff_corrTI_RSD_Ref'] = (inputdata['corrTI_RSD_TI']
                                               - inputdata['Ref_TI'])
        # calculating the error for each timestamp
        inputdata['TI_error_corrTI_RSD_Ref'] = (
            inputdata['TI_diff_corrTI_RSD_Ref']/inputdata['Ref_TI'])
        TI_MBE_j_corrTI_rsd_ref, TI_MBE_jp5_corrTI_rsd_ref = (
            get_stats_per_WSbin(inputdata, 'TI_error_corrTI_RSD_Ref'))
        TI_Diff_j_corrTI_rsd_ref, TI_Diff_jp5_corrTI_rsd_ref = (
            get_stats_per_WSbin(inputdata, 'TI_diff_corrTI_RSD_Ref'))
        TI_MBE_j_.append([TI_MBE_j_corrTI_rsd_ref, TI_MBE_jp5_corrTI_rsd_ref])
        TI_Diff_j_.append([TI_Diff_j_corrTI_rsd_ref,
                           TI_Diff_jp5_corrTI_rsd_ref])
    else:
    ...
    # this is checking if the corrected WS method was used for the
    # wind speed and TI
    if 'corrWS_RSD_TI' in inputdata.columns:
        # caliculating the diff in ti for each timestamp
        inputdata['TI_diff_corrWS_RSD_Ref'] = (
            inputdata['corrWS_RSD_TI']-inputdata['Ref_TI'])
        # calculating the error for each timestamp
        inputdata['TI_error_corrWS_RSD_Ref'] = (
            inputdata['TI_diff_corrWS_RSD_Ref']/inputdata['Ref_TI'])
        TI_MBE_j_corrWS_rsd_ref, TI_MBE_jp5_corrWS_rsd_ref = (
            get_stats_per_WSbin(inputdata, 'TI_error_corrWS_RSD_Ref'))
        TI_Diff_j_corrWS_rsd_ref, TI_Diff_jp5_corrWS_rsd_ref = (
            get_stats_per_WSbin(inputdata, 'TI_diff_corrWS_RSD_Ref'))
        TI_MBE_j_.append([TI_MBE_j_corrWS_rsd_ref, TI_MBE_jp5_corrWS_rsd_ref])
        TI_Diff_j_.append([TI_Diff_j_corrWS_rsd_ref,
                           TI_Diff_jp5_corrWS_rsd_ref])
    else:
        TI_MBE_j_.append(pd.DataFrame([None, None]))
        TI_Diff_j_.append(pd.DataFrame([None, None]))
    ...
    # calculating the error for each timestamp
    inputdata['TI_error_Ane2_Ref'] = (inputdata['TI_diff_Ane2_Ref']
                                      / inputdata['Ref_TI'])
    TI_MBE_j_Ane2_Ref, TI_MBE_jp5_Ane2_Ref = (
        get_stats_per_WSbin(inputdata, 'TI_error_Ane2_Ref'))
    TI_Diff_j_Ane2_Ref, TI_Diff_jp5_Ane2_Ref = (
        get_stats_per_WSbin(inputdata, 'TI_diff_Ane2_Ref'))
...
def get_TI_bybin(inputdata):
    ...
    # input data and using that for getting the results.
    if 'corrTI_RSD_TI' in inputdata.columns:
        corrTI_RSD_TI_j, corrTI_RSD_TI_jp5 = (
            get_stats_per_WSbin(inputdata, 'corrTI_RSD_TI'))
        results.append([corrTI_RSD_TI_j, corrTI_RSD_TI_jp5])
    else:
        results.append(pd.DataFrame([None, None]))

    # get the bin wise stats for both diff and error between RSD
    # corrected for ws and Ref

    # this is checking if the corrected WS method was used for the
    # wind speed and TI
    if 'corrWS_RSD_TI' in inputdata.columns:
        corrWS_RSD_TI_j, corrWS_RSD_TI_jp5 = (
            get_stats_per_WSbin(inputdata, 'corrWS_RSD_TI'))
...
def get_stats_inBin(inputdata_m, start, end):
    # this was discussed in the meeting , but the results template
    # didn't ask for this.
    inputdata = inputdata_m.loc[(inputdata_m['Ref_WS'] > start)
                                & (inputdata_m['Ref_WS'] <= end)].copy()
    # caliculating the diff in ti for each timestamp
    inputdata['TI_diff_RSD_Ref'] = inputdata['RSD_TI']-inputdata['Ref_TI']
    # calculating the error for each timestamp
    inputdata['TI_error_RSD_Ref'] = (inputdata['TI_diff_RSD_Ref']
    ...
    results = pd.DataFrame([TI_error_RSD_Ref_Avg, TI_error_RSD_Ref_Std,
                            TI_diff_RSD_Ref_Avg, TI_diff_RSD_Ref_Std],
                           columns=['RSD_Ref'])
    # this is checking if corrected TI windspeed is present in the input
    # data and using that for getting the results.
    if 'corrTI_RSD_TI' in inputdata.columns:
        ...
        inputdata['TI_error_corrTI_RSD_Ref'] = (
            inputdata['TI_diff_corrTI_RSD_Ref'] / inputdata['Ref_TI'])
        TI_error_corrTI_RSD_Ref_Avg = (inputdata['TI_error_corrTI_RSD_Ref']
                                       .mean())
        TI_error_corrTI_RSD_Ref_Std = (inputdata['TI_error_corrTI_RSD_Ref']
                                       .std())
        ...
        results['CorrTI_RSD_Ref'] = [TI_error_corrTI_RSD_Ref_Avg,
    ...
    if 'corrWS_RSD_TI' in inputdata.columns:
        # caliculating the diff in ti for each timestamp
        inputdata['TI_diff_corrWS_RSD_Ref'] = (inputdata['corrWS_RSD_TI']
                                               - inputdata['Ref_TI'])
        # calculating the error for each timestamp
        inputdata['TI_error_corrWS_RSD_Ref'] = (
            inputdata['TI_diff_corrWS_RSD_Ref']/inputdata['Ref_TI'])
        TI_error_corrWS_RSD_Ref_Avg = (inputdata['TI_error_corrWS_RSD_Ref']
                                       .mean())
        TI_error_corrWS_RSD_Ref_Std = (inputdata['TI_error_corrWS_RSD_Ref']
                                       .std())
        ...
        results['corrWS_RSD_Ref'] = [TI_error_corrWS_RSD_Ref_Avg,
                                     TI_error_corrWS_RSD_Ref_Std,
                                     TI_diff_corrWS_RSD_Ref_Avg,
                                     TI_diff_corrWS_RSD_Ref_Std]
    else:
    ...
    results['Ane2_Ref'] = [TI_error_Ane2_Ref_Avg, TI_error_Ane2_Ref_Std,
                           TI_diff_Ane2_Ref_Avg, TI_diff_Ane2_Ref_Std]
    results.index = ['TI_error_mean', 'TI_error_std', 'TI_diff_mean',
                     'TI_diff_std']
...
def get_representative_TI_15mps(inputdata):
    # this is the represetative TI, this is currently only done at a
    # 1m/s bins not sure if this needs to be on .5m/s
...
def write_all_resultstofile(reg_results, TI_MBE_j_, TI_Diff_j_,  # noqa C901
                            rep_TI_results, TIbybin, count, total_stats,
                            filename):
    wb = Workbook()
    ws = wb.active
    # regression results
    ...
    for val in TI_MBE_j_:
        for val1 in val:
            try:
                ...
                print('Could not write a row in TI_MBE_j_, TI Corrected or '
                      'WS corrected windspeeds are not provided so not '
                      'writing them')
            ...
            for val1 in val:
                try:
                    ...
                    print('Could not write a row in TI_Diff_j_, '
                          'TI Corrected or WS corrected windspeeds are '
                          'not provided so not writing them')
    rownumber += 6
    for val in TIbybin:
        for i in val:
            try:
    ...
    rownumber += 8
    headers_stats = dict(zip([0, 1, 2], ['Total Bin Stats',
                                         'Below nominal Stats',
                                         'Above nominal Status']))
...
def get_inputfiles():
    ...
    parser.add_argument("input_filename",
                        help="print this requires the input filename")
    parser.add_argument("config_file",
                        help="this reuquires the excel configuration file")
    parser.add_argument("results_file",
                        help="this reuquires the excel results file")
...
if __name__ == '__main__':
    ...
    write_all_resultstofile(reg_results, TI_MBE_j_, TI_Diff_j_,
                            rep_TI_results, TIbybin, count, total_stats,
                            results_filename)
